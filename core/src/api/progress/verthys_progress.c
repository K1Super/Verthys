 ...
 * 仅在 Windows 平台编译（整体由 #ifdef _WIN32 包裹）。
 *
 * 线程安全：
 ...
 *   - 回调在消费线程中异步执行，DLL 不解析 user_data 内容
 *
 * 性能：本函数仅做 O(1) 环形缓冲区入队（<1μs），
...
#ifdef _WIN32
    /* 优先写入无锁环形缓冲区（O(1) <1μs），独立消费线程异步推送 */
