 ...
 *
 * 扫描流程变更要点：
 *   - CancellationToken + scan_abort 回滚游标；scan_close 触发 cancel + 等待退出
 *   - 擦除顺序由 LockedBuffer Drop 保证（先 zeroize while VirtualLock'd → 再 VirtualUnlock）
 *   - ScanSessionState 状态机（Active/Failed/Closed），操作经状态转移
 *   - 泛型 ScanPipeline<T: ScanRecord>（全量与摘要共用）
 *   - shm_name 正则校验 ^[a-zA-Z0-9_\-]{1,64}$ + TimeoutConfig 超时提取
 *   - 每批校验 records.len() == worker record_count，不一致熔断 + 审计 + 通知 worker 重置
 *
...
/* ------------------------------------------------------------------ *
 * 超时配置（提取为常量，对应 TimeoutConfig::DEFAULT）         *
 * ------------------------------------------------------------------ */

/// 预取任务关闭超时（对应 TimeoutConfig.scan_prefetch_shutdown）
const PREFETCH_SHUTDOWN_TIMEOUT: Duration = Duration::from_secs(2);
/// scan_open 超时（对应 TimeoutConfig.scan_open）
const SCAN_OPEN_TIMEOUT: Duration = Duration::from_secs(60);
/// scan_next 超时（对应 TimeoutConfig.scan_next）
const SCAN_NEXT_TIMEOUT: Duration = Duration::from_secs(60);
/// scan_close 超时（对应 TimeoutConfig.scan_close）
const SCAN_CLOSE_TIMEOUT: Duration = Duration::from_secs(30);
/// 预取内部二级超时（对应 TimeoutConfig.scan_prefetch）
...
///   5. 校验 records.len() == worker record_count
...
pub async fn verthys_scan_open(
    ...
    if let Some(mut old) = old_scan {
        ...
        // 通知 worker 取消旧游标（发 scan_abort 回滚）
    ...
    // 6. 存入 ScanPipelineState（session_state = Active）
    {
        let mut guard = state.lock_scan_state();
...
///   2. 检查 session_state == Active，Failed/Closed 拒绝
///   3. Drop 旧 current_buffer（LockedBuffer Drop: 先擦除后解锁）
...
pub async fn verthys_scan_next(
    ...
    // RAII 保证擦除顺序，替代 virtual_unlock + secure_zero
    ...
    // 7. 若未遍历结束，通过同一预取任务发送下一轮 Fetch（长期任务复用）
    ...
    // 8. 存回 ScanPipelineState（保持 Active）
...
/// 关闭扫描游标，释放所有资源
///
/// scan_close 触发 cancel + 等待任务自退出（非 abort）
///
/// 流程：
...
pub async fn verthys_scan_close(
    ...
    if let Some(mut scan) = scan_opt {
        ...
        // LockedBuffer Drop 先 zeroize 后 VirtualUnlock
 ...
 *                                                                    *
 * 全量与摘要共用 ScanPipelineState<T> + PrefetchTaskHandle<T>*
 * 与全量扫描同构的校验/状态机/审计                      *
 * ------------------------------------------------------------------ */
